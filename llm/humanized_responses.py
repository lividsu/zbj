...
class DesignerPersonality:
    ...
    def get_reflection_need_improve(self, score: int, issues: List[str]) -> str:
        """获取需要优化的反思回复"""
        issues_str = "、".join(issues[:3]) if issues else "构图或色彩细节"
        prompt = f"你是一名对设计质量要求极高的资深平面设计师。你给自己的作品打了{score}分（10分满分），发现了问题：{issues_str}。请用自然口语（30字以内）告诉用户你不够满意，决定再优化一版。直接输出回复。"
        base = self._generate(prompt)
        return f"🤔 自检得分：{score}/10\n{base}"

    def get_reflection_satisfied(self, score: int, analysis: str = "") -> str:
        """获取满意的反思回复"""
        prompt = f"你是一名资深平面设计师，给自己的作品打了{score}分（10分满分），觉得构图、色彩、质量都不错。请用自然自信的口语（20字以内）告诉用户审查通过了。直接输出回复。"
        base = self._generate(prompt)
