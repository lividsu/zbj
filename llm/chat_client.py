...
class ChatHandler:
    ...
    def reflect_on_generated_image(
        ...
        try:
            ...
            reflection_prompt = f"""你是一名严格的资深平面设计师，正在评审 AI 生成的设计图片。请评估第一张图片（AI生成的图片）是否满足用户的设计需求。

用户的原始请求: {original_prompt}
{ref_hint}

请从以下维度评估生成的图片:
1. 内容准确性: 是否包含用户要求的所有视觉元素？文字、图形、人物是否符合描述？
2. 构图与版式: 视觉重心是否合理？空间布局、留白运用是否得当？
3. 色彩运用: 配色是否和谐？色调是否符合主题氛围和用户期望？
4. 视觉层次: 信息层级是否清晰？主次关系是否突出？
5. 图片质量: 是否有明显瑕疵、变形、人体结构错误或 AI 生成痕迹？
6. 整体设计感: 是否具备专业设计水准，达到可交付水平？
...
- improved_prompt 应该是一个更清晰、更详细的图片生成指令，需明确：构图方式、色彩风格、视觉重心、氛围感等设计细节
